...
export const StocksCalc = () => {
  const [expectedReturn, setExpectedReturn] = useState(12); 
  const [investAmount, setInvestAmount] = useState(5000);  
  const [years, setYears] = useState(10);   


  const calculateSIP = (P, rAnnual, tYears) => {
  ...
  const expectedFinalValue = calculateSIP(investAmount, expectedReturn, years);
    ...
    <div className="modern-calculator">
      ...
      <div className="calculator-results">
        ...
        <Typography variant="body1" className="result-value">
          Expected Final Value (approx): ₹{expectedFinalValue.toLocaleString()}
        </Typography>
        <Typography variant="body1" className="result-value">
            Expected Profit (approx): ₹{(expectedFinalValue - totalInvestment).toFixed(0)}
        </Typography>
      </div>
    </div>
  );
